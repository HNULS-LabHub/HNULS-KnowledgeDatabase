...
- [x] 1. IPC 类型定义与配置类型扩展
  - [x] 1.1 在 `knowledge-graph-ipc.types.ts` 中新增嵌入相关 IPC 消息类型
  ...
  - [x] 1.2 在 `knowledge-config.types.ts` 的 `KnowledgeGraphModelConfig` 中新增 `embeddingBatchSize?: number` 和 `embeddingMaxTokens?: number` 字段
    - _需求: 10A.1_

- [x] 2. Entity Schema 扩展与 Upsert 修改
  - [x] 2.1 修改 `graph-schema/index.ts`，在 entity 表 Schema 中新增 embedding/embedding_hash/embedding_at 字段定义
  ...
  - [x] 2.2 修改 `graph-upsert.ts`，在 entity UPSERT 语句中追加 `embedding = NONE, embedding_hash = ''`
...
- [x] 3. EmbeddingScheduler 核心实现
  - [x] 3.1 新建 `core/embedding-scheduler.ts`，实现 EmbeddingScheduler 类骨架
  ...
  - [x] 3.2 实现 `buildEmbeddingText` 纯函数
...
- [x] 4. Checkpoint - 确保核心模块编译通过
  - 确保所有测试通过，如有问题请向用户确认。

- [x] 5. IPC 集成与消息路由
  - [x] 5.1 修改 `message-handler.ts`，新增 EmbeddingScheduler 依赖注入和消息路由
  ...
  - [x] 5.2 修改 `entry.ts`，初始化 EmbeddingScheduler 并注入到 MessageHandler
...
- [x] 6. 主进程桥接层扩展
  - [x] 6.1 修改 `knowledge-graph-bridge/index.ts`，新增嵌入相关方法和消息处理
  ...
  - [x] 6.2 在主进程中实现 build-completed → trigger-embedding 的自动触发逻辑
...
- [x] 9. 配置 Store 适配
  - [x] 9.1 修改 `knowledge-config.store.ts` 的 createKgConfig 方法，为新建 KG 配置填充 embeddingBatchSize=20 和 embeddingMaxTokens=1500 默认值
    - _需求: 10A.2, 10A.3_
