...
graph TB
    subgraph "Main Process 主进程"
        IPCHandler["Embedding IPC Handler<br/>转发指令"]
        ModelConfig["ModelConfigService<br/>提供商配置"]
    end

    subgraph "Utility Process: Embedding Engine 嵌入引擎"
        EntryPoint["entry.ts<br/>进程入口"]

        subgraph "核心层"
            TaskManager["TaskManager 任务管理器<br/>文档→Chunks 两级表"]
            Scheduler["Scheduler 调度器<br/>并发控制 + 队列"]
        end

        subgraph "执行层"
            ChannelManager["ChannelManager 通道管理器<br/>多源级联 + 熔断"]
            OpenAIClient["OpenAI Client<br/>SDK 封装"]
        end

        subgraph "监控层"
            ProgressTracker["ProgressTracker 进度追踪<br/>计算完成率"]
            TaskMonitorBridge["TaskMonitor Bridge<br/>回调全局监控"]
        end
    end

    subgraph "Global Monitor 全局监控"
        TaskRegistry["TaskRegistry<br/>任务生命周期管理"]
    end

    IPCHandler -->|IPC: start-embed| EntryPoint
    ModelConfig -->|提供 Channel 配置| ChannelManager
...
```mermaid
sequenceDiagram
    participant Renderer as 渲染进程<br/>(Vue UI)
    participant Main as 主进程<br/>(IPC Handler)
    participant Embed as 嵌入引擎<br/>(Utility Process)
    participant TaskMon as 全局监控<br/>(Global Monitor)

    Renderer->>Main: 提交嵌入任务<br/>window.api.embedding.submit(params)
    Main->>TaskMon: 创建文档级任务<br/>taskMonitor.createTask()
    TaskMon-->>Main: 返回 taskHandle
    Main->>Embed: MessagePort 转发任务<br/>{ type: 'embed:start', data: {...} }

    Embed->>Embed: TaskManager.createDocumentTask()
    Embed->>Embed: TaskManager.addChunks(chunks)
    Embed->>Embed: Scheduler.enqueue(chunks)

    Main-->>Renderer: 返回 taskId

    loop 并发处理
        Embed->>Embed: Scheduler 取出 chunk
        Embed->>Embed: ChannelManager.embedWithFallback()
        Embed->>Embed: ProgressTracker.updateChunkCompleted()
        Embed->>TaskMon: 回调进度<br/>{ taskId, progress, meta }
        TaskMon-->>Renderer: 前端监听进度更新
    end

    Embed->>TaskMon: 任务完成<br/>taskHandle.complete()
```
