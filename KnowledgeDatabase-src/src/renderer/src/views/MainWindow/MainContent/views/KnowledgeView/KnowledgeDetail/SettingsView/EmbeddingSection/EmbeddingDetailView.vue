      ...
      <div class="flex items-center justify-between">
        <div class="flex flex-col gap-1">
          <div class="flex items-center gap-2">
            ...
            <span class="px-2 py-0.5 bg-blue-100 text-blue-700 text-[10px] font-bold uppercase rounded tracking-wider">轮询配置模式</span>
          </div>
    ...
    <div class="flex-1 overflow-y-auto p-8 space-y-8 min-h-0">
      ...
      <div class="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-4">
        ...
        <div class="flex-1 text-sm text-amber-900 py-1">
          ...
          <p class="opacity-80">
            在一个配置方案中，所有的候选模型必须产出<strong>相同长度</strong>且针对<strong>相同词义空间</strong>优化的向量。通常建议仅选择同一个预设系列（如都是 text-embedding-3-small）的不同提供商节点。
          </p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs text-slate-400 font-normal">({{ localCandidates.length }} 个节点)</span>
          </div>
          <button
        ...
        <!-- Empty State -->
        <div v-if="localCandidates.length === 0" class="bg-white border-2 border-dashed border-slate-200 rounded-2xl py-12 flex flex-col items-center justify-center text-slate-400">
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex items-center gap-4">
              <div class="w-8 h-8 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-500 font-mono text-xs font-bold transition-colors">
                {{ index + 1 }}
              </div>
              <div>
                <div class="font-bold text-slate-900 flex items-center gap-2">
                  {{ getModelName(candidate) }}
                  <span v-if="index === 0" class="text-[9px] px-1.5 py-0.5 bg-green-100 text-green-700 rounded-md font-bold uppercase tracking-tight">首选</span>
                </div>
                <div class="text-xs text-slate-400 flex items-center gap-1.5 mt-0.5">
                  <span class="px-1.5 py-0.5 bg-slate-100 rounded text-slate-500 font-medium">{{ getProviderName(candidate) }}</span>
