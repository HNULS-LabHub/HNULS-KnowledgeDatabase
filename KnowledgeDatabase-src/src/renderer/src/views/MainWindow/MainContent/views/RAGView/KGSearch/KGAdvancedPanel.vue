<template>
  <div v-if="kgStore.advancedExpanded" class="kg-advanced-panel glass-card p-4 flex-shrink-0 overflow-y-auto max-h-[220px]">
    <div class="grid grid-cols-2 gap-x-6 gap-y-3 text-xs">
      ...
      <div class="col-span-2">
        ...
        <div class="flex items-center gap-3">
          <label class="flex items-center gap-1.5 cursor-pointer">
            <input type="checkbox" v-model="kgStore.keywordUseLLM" class="rounded border-slate-300 text-teal-500 focus:ring-teal-500 w-3.5 h-3.5" />
      ...
      <div>
        ...
        <div class="flex flex-col gap-1.5">
          <div class="flex items-center gap-2">
            ...
            <input v-model.number="kgStore.vectorSearch.entityTopK" type="number" min="1" max="200" class="w-16 px-1.5 py-0.5 border border-slate-200 rounded text-xs text-center focus:outline-none focus:border-teal-300" />
          </div>
          <div class="flex items-center gap-2">
            <span class="text-slate-400 w-20">Relation TopK</span>
            <input v-model.number="kgStore.vectorSearch.relationTopK" type="number" min="1" max="200" class="w-16 px-1.5 py-0.5 border border-slate-200 rounded text-xs text-center focus:outline-none focus:border-teal-300" />
          </div>
          <div class="flex items-center gap-2">
            <span class="text-slate-400 w-20">Chunk TopK</span>
            <input v-model.number="kgStore.vectorSearch.chunkTopK" type="number" min="1" max="500" class="w-16 px-1.5 py-0.5 border border-slate-200 rounded text-xs text-center focus:outline-none focus:border-teal-300" />
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="flex flex-col gap-1.5">
          <div class="flex items-center gap-2">
            ...
            <input v-model.number="kgStore.graphTraversal.maxDepth" type="number" min="1" max="5" class="w-16 px-1.5 py-0.5 border border-slate-200 rounded text-xs text-center focus:outline-none focus:border-teal-300" />
          </div>
          <div class="flex items-center gap-2">
            <span class="text-slate-400 w-20">最大邻居</span>
            <input v-model.number="kgStore.graphTraversal.maxNeighbors" type="number" min="1" max="50" class="w-16 px-1.5 py-0.5 border border-slate-200 rounded text-xs text-center focus:outline-none focus:border-teal-300" />
          </div>
        </div>
      </div>
      ...
      <div class="col-span-2">
        <label class="flex items-center gap-1.5 cursor-pointer mb-1.5">
          <input type="checkbox" v-model="kgStore.rerankEnabled" class="rounded border-slate-300 text-teal-500 focus:ring-teal-500 w-3.5 h-3.5" />
