        ...
        >
          ...
          <div class="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
            ...
            <div class="space-y-4">
              ...
              <div class="space-y-3">
                <label class="text-sm font-semibold text-slate-700 block">选择常用模型预设 (快速填写)</label>
                <div class="grid grid-cols-2 gap-3">
                  <button
                    ...
                    :class="form.presetId === preset.id ? 'bg-blue-50 border-blue-300 ring-1 ring-blue-300' : 'bg-white border-slate-200 hover:border-blue-200'"
                    @click="applyPreset(preset)"
                  >
                    <span class="text-xs font-bold text-slate-400 group-hover:text-blue-500 mb-1 uppercase tracking-wider">{{ preset.id.includes('openai') ? 'OpenAI' : 'OpenSource' }}</span>
                    <span class="text-sm font-semibold text-slate-900 mb-1">{{ preset.name }}</span>
                    <span class="text-[10px] text-slate-400">推荐维度: {{ preset.defaultDimensions }}</span>
                  </button>
                </div>
              </div>
