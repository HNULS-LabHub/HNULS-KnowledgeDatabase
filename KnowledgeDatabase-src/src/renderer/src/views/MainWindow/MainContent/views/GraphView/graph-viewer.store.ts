...
export const useGraphViewerStore = defineStore('graph-viewer', () => {
  ...
  function setupEventListeners(): void {
    ...
    // 监听完成
    cleanupFns.push(
      api.onGraphDataComplete((sessionId) => {
    ...
    // 监听错误
    cleanupFns.push(
      api.onGraphDataError((sessionId, error) => {
        if (sessionId !== currentSessionId.value) return
    ...
    // 监听取消
    cleanupFns.push(
      api.onGraphDataCancelled((sessionId) => {
        if (sessionId !== currentSessionId.value) return
  ...
  watch(
    () => knowledgeLibraryStore.knowledgeBases,
    async (kbs) => {
      for (const kb of kbs) {
        if (!loadedKbIds.value.has(kb.id)) {
          await knowledgeConfigStore.loadConfig(kb.id)
