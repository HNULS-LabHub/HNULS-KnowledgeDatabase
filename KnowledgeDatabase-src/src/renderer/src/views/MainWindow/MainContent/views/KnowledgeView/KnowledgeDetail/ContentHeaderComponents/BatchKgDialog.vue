          ...
          >
            ...
            <div class="px-6 py-5 border-b border-slate-100">
              <div class="flex items-center justify-between">
                ...
                <button class="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg transition-all" @click="handleClose">
...
const defaultKgConfigId = computed(() =>
  configStore.getDefaultKgConfigId(props.knowledgeBaseId)
)
...
function handleClose(): void { emit('update:modelValue', false) }
...
<style scoped>
.dialog-fade-enter-active { transition: opacity 250ms cubic-bezier(0.4, 0, 0.2, 1); }
.dialog-fade-leave-active { transition: opacity 200ms cubic-bezier(0.4, 0, 1, 1); }
.dialog-fade-enter-from, .dialog-fade-leave-to { opacity: 0; }
.dialog-scale-enter-active { transition: transform 350ms cubic-bezier(0.34, 1.56, 0.64, 1), opacity 250ms cubic-bezier(0.4, 0, 0.2, 1); }
.dialog-scale-leave-active { transition: transform 250ms cubic-bezier(0.4, 0, 1, 1), opacity 200ms cubic-bezier(0.4, 0, 1, 1); }
.dialog-scale-enter-from { transform: scale(0.95) translateY(-10px); opacity: 0; }
.dialog-scale-leave-to { transform: scale(0.98) translateY(5px); opacity: 0; }
</style>
