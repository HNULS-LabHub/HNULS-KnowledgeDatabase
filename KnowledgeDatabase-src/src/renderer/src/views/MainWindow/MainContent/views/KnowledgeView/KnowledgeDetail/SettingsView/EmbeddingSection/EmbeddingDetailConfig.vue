  ...
  <div class="kb-embedding-detail flex flex-col h-full bg-white">
    ...
    <div class="p-6 space-y-6 flex-1 overflow-y-auto">
      ...
      <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex gap-3">
        ...
        <div class="text-sm text-blue-900">
          ...
          <p class="text-blue-700/80">
            您可以配置多个提供商的 <strong>{{ preset.name }}</strong> 模型作为候选项。系统将按照列表顺序进行轮询或负载均衡。
            <br />
            <span class="font-semibold text-red-500">注意：</span>
            请务必确保选中的模型实际上是同一种模型架构（{{ preset.id }}），否则会导致生成的向量不兼容，严重影响检索效果。
          </p>
        </div>
      </div>
      ...
      <div class="space-y-3">
        ...
        <div v-if="localCandidates.length === 0" class="text-center py-8 bg-slate-50 rounded-xl border border-dashed border-slate-200 text-slate-400">
          暂无配置的模型，请点击右上角添加。
        </div>
        ...
        <transition-group name="list" tag="div" class="space-y-2">
          ...
          >
            <div class="flex items-center gap-3">
              <div class="w-6 h-6 rounded bg-slate-100 flex items-center justify-center text-slate-400 font-mono text-xs">
                {{ index + 1 }}
              </div>
              <div>
      ...
      <div class="pt-6 border-t border-slate-100">
         <label class="block text-sm font-medium text-slate-700 mb-2">
          嵌入维度
          <span class="text-slate-400 font-normal">(可选，默认 {{ preset.defaultDimensions }})</span>
        </label>
        <input
