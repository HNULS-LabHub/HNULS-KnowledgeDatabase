<template>
  <div
    class="ts-model-result-card bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col"
  >
    <!-- 头部：模型名 + 状态 -->
    <div class="px-3 py-2 border-b border-slate-100 flex items-center justify-between bg-slate-50">
      ...
      <div class="flex items-center gap-2 flex-shrink-0">
        ...
        <span
          ...
          >完成</span
        >
        <span v-else-if="result.status === 'error'" class="text-[10px] text-red-600 font-medium"
          >错误</span
        >
      </div>
    </div>
    ...
    <!-- 性能指标 -->
    <div
      class="px-3 py-1.5 border-b border-slate-100 flex items-center gap-3 text-[10px] text-slate-500 bg-slate-50/50"
    >
    ...
    <div class="flex-1 overflow-auto p-3 space-y-2 max-h-[400px]">
      ...
      <div v-if="result.reasoning" class="space-y-1">
        ...
        <pre
          class="p-2 bg-purple-50 border border-purple-200 rounded text-[10px] text-purple-800 whitespace-pre-wrap overflow-auto max-h-[120px] font-mono"
      ...
      <div class="space-y-1">
        ...
        <div
          ...
          class="p-2 bg-red-50 border border-red-200 rounded text-[10px] text-red-700"
        >
          {{ result.error }}
        </div>
      </div>
    ...
    <div
      v-if="result.usage"
      class="px-3 py-1.5 border-t border-slate-100 bg-slate-50 text-[10px] text-slate-500 flex gap-3"
    >
