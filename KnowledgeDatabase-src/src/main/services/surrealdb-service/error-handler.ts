...
export class ErrorHandler {
   ...
   * 处理运行时错误
   */
  static handleRuntimeError(error: Error, hookSystem: HookSystem, context?: string): void {
   ...
   * 处理关闭错误
   */
  static handleShutdownError(error: Error, process?: ChildProcess | null): void {
  ...
  static handleProcessCrash(
    ...
    const crashInfo = signal ? `killed by signal ${signal}` : `exited with code ${code}`
